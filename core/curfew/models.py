...
class User(SQLModel, table=True):
    ...
    # Argon2 hash of the user's password. NULL means "no password set; cannot
    # log in" — covers users created before per-user auth shipped and any
    # future SSO-only users.
...
class UserSession(SQLModel, table=True):
    """Per-user session row for cookie auth.

    The cookie value is the row's ``id`` — an opaque random token generated by
    ``secrets.token_urlsafe(32)``. The DB row is the single source of truth
    for "is this session valid?" so logout / revocation is just a row delete.

    Class is named ``UserSession`` (table ``sessions``) so it doesn't shadow
    SQLModel's own ``Session`` class used everywhere for DB transactions.
    """
