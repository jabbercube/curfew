...
| `managed` | Bool, defaults to `true`. Whether lock rules apply to this user. Independent of `role` — a teen `manager` could be `managed: true` (has lock control over siblings *and* their own rules apply); a houseguest could be `member, managed: false` (no powers, not subject to rules). When `false`, `GET /v1/users/{user}/status` always returns `{locked: false, reasons: []}` and the rule pipeline is skipped. PATCHing `managed: false` on a currently-locked user implicitly clears the `manual_lock` flag (audited as `user.unlock` with `reason: "managed_to_unmanaged"`) and dispatches plugin reconcile so external state follows — the kernel maintains the invariant that an unmanaged user is never locked at the DB level. The reverse direction (PATCHing `managed: true`) is a pure flag flip; the kernel never auto-locks. Operators typically opt out (`--managed false`) for adult managers and admins, and for guests; a teen `manager` who's also subject to rules stays `managed: true`. |
